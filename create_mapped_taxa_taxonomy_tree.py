...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print "python "+sys.argv[0]+" inlist taxonomy outfile"
        sys.exit(0)
    id_num = {}
    ...
    if targetid in id_num:
        ...
        #for newick
        #nodes[targetid].label += '[&!label="'+str(id_num[targetid])+'"]'
    count = 0
    while len(stack) > 0:
        tempid = stack.pop()
        if tempid not in cid:
            continue
        if rid[tempid] == "genus" or rid[tempid] == "species":
            continue
        count += 1
        if count >maxnodes:
            break
        nodes[tempid] = Node()
        nodes[tempid].label = nid[tempid].replace("(","_").replace(")","_").replace(" ","_").replace(";","_").replace(":","_")
    ...
    outfile.write(nodes[targetid].get_json_repr(datatouse))
    outfile.close()
    #print '#NEXUS\nbegin trees;\n'
